...
payment, and reconciles **bank-transfer slips** automatically through the
**SlipOk** verification API.
...
| 📥 **Slip Verify** | Drag-and-drop a JPG/PNG slip → SlipOk verifies it → the amount is matched to the player who owes it → their row flips `Pending → Paid`. |
...
#   → paste your Google service-account JSON fields and SlipOk endpoint/key
